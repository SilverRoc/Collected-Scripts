...
# ------------------------------------------------------
# [5/6] Updating DNS configuration based on region
# ------------------------------------------------------
...
echo ""
echo "DNS Configuration Mode (press Enter to use default)"
echo "-------------------------------------------------------"
echo "1) Static resolv.conf (manual configuration)  [DEFAULT]"
echo "2) systemd-resolved (Ubuntu22/Debian12 recommended)"
echo "-------------------------------------------------------"
...
if [ "$DNS_MODE" = "2" ]; then
    ...
    if systemctl list-unit-files | grep -q "^systemd-resolved"; then
        ...
        echo "systemd-resolved not supported, switching to static mode."
        DNS_MODE="1"
    fi
fi

if [ "$DNS_MODE" = "1" ]; then
    echo "Selected: Static resolv.conf mode (default)"
...
# ------------------------------------------------------
# [6/6] Auto-detect OS and ask user whether to modify hostname
# ------------------------------------------------------
echo "[6/6] Detecting OS version..."
...
echo ""
echo "Change hostname to '$AUTO_HOSTNAME'?"
echo "  y / yes  = apply new hostname"
echo "  n / no   = keep current hostname (default)"
echo "-------------------------------------------------------"
read -p "Apply hostname change? [y/N, default=N] : " CHANGE_HOSTNAME

# default: No
CHANGE_HOSTNAME=${CHANGE_HOSTNAME,,}  # lowercase

if [[ "$CHANGE_HOSTNAME" == "y" || "$CHANGE_HOSTNAME" == "yes" ]]; then
